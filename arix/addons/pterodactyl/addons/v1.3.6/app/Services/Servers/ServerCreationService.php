...
class ServerCreationService
{
    ...
    public function handle(array $data, ?DeploymentObject $deployment = null): Server
    {
        ...
        if (empty($data["node_id"])) {
            Assert::false(
                empty($data["allocation_id"]),
                "Expected a non-empty allocation_id in server creation data.",
            );

            $data["node_id"] = Allocation::query()->findOrFail(
                $data["allocation_id"],
            )->node_id;
        }
        ...
        if (empty($data["nest_id"])) {
            Assert::false(
                empty($data["egg_id"]),
                "Expected a non-empty egg_id in server creation data.",
            );

            $data["nest_id"] = Egg::query()->findOrFail(
                $data["egg_id"],
            )->nest_id;
        }
        ...
        /** @var Server $server */
        $server = $this->connection->transaction(function () use (
            $data,
            $eggVariableData,
        ) {
    ...
    private function createModel(array $data): Server
    {
        ...
        $model = $this->repository->create([
            ...
            "skip_scripts" =>
                Arr::get($data, "skip_scripts") ?? isset($data["skip_scripts"]),
    ...
    private function storeAssignedAllocations(Server $server, array $data): void
    {
        ...
        if (
            isset($data["allocation_additional"]) &&
            is_array($data["allocation_additional"])
        ) {
     ...
     */
    private function storeEggVariables(
        Server $server,
        Collection $variables,
    ): void {
        $records = $variables
            ->map(function ($result) use ($server) {
                return [
                    "server_id" => $server->id,
